...
class NodeEmbedding:
    ...
    def get_embeddings(self, method, feat=None, embed_size=128):
        ...
        if method == 'SVD':
            ...
            sigma = self.args.sigma if self.args else 2.0
            cn_percentile = self.args.cn_percentile if self.args else 95.0
            ...
            q=q,
            sigma=sigma,
            cn_percentile=cn_percentile,
            use_weighting=not self.args.no_weighting if self.args else True)
    ...
    def save_embeddings(self, filename):
        """Save embeddings to file in node2vec format"""
        if self.embeddings is None:
            raise ValueError("No embeddings to save. Run get_embeddings() first.")
    ...
    def load_embeddings(self, filename):
        ...
        with open(filename, 'r') as f:
            lines = f.readlines()
            # First line: node_num embed_size
            node_num, embed_size = map(int, lines[0].strip().split())
            self.embed_size = embed_size
            for line in lines[1:]:
