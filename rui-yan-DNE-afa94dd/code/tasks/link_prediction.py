...
class LinkPredictor(object):
    ...
    def train_and_evaluate(self, method, node_subjects=None, cv_fold=5, n_trials=5):
        ...
        for trial in range(n_trials):
            ...
            if node_subjects is not None:
                ...
                embeddings = embedding_model.get_embeddings(method, feat=feat, embed_size=self.embed_size)
            else:
            ...
            print(f"  [operator selected] trial={trial}: {best_result['binary_operator'].__name__}")  # add this line
    ...
    def train(self, X, Y, op, cv_fold):
        ...
        clf.fit(X_embed, Y)
        print(f"    [C selected] operator={op.__name__}: C={clf.named_steps['clf'].C_}")  # add this
        return clf
